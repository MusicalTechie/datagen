# ==================================================
# = File:        DataGen_1430_Acquire_CUSTOMER-SITESs_from_Oracle_via_REST.py
# = Purpose:     Execute a REST API call to acquire CUSTOMER-SITE (customer account sites) information from a designated Oracle Cloud Fusion environment
# ==================================================
# = Notes, Warnings, Requirements:
# = ------------------------------------------------
# =  - Uses config: v_parm_fusion_*, v_parm_path_customersites_from_source.
...
v_current_procedure_name = 'Acquire CUSTOMER-SITEs from Oracle via REST'
...
v_api_rest_endpoint = "/fscmRestApi/resources/11.13.18.05/customerAccountSitesLOV"
...
# --- Acquire CUSTOMER-SITEs in blocks until no more records ---
while True:
...
# --- Write all customer-sites and attributes to quote-delimited CSV (overwrite if exists) ---
v_output_path = v_parm_path_customersites_from_source
...
if v_api_rest_response_customers_array:
    ...
    logging.debug("Wrote " + str(len(v_api_rest_response_customers_array)) + " customer-sites to " + str(v_output_path))
else:
    with open(v_output_path, mode="w", newline="", encoding="utf-8") as v_f:
        v_writer = csv.writer(v_f, quoting=csv.QUOTE_ALL, delimiter=",")
        v_writer.writerow([])
    logging.debug("No customer-sites to write; created empty file: " + str(v_output_path))

logging.debug("Total CUSTOMER-SITE rows acquired via REST: " + str(len(v_api_rest_response_customers_array)))
logging.debug("\n" * 2)
logging.debug("==================================================")
