...
if 1 == 1:
    ...
    if 1 == 1:
        if int(v_parm_flag_loglevel) > 0:
            logging.debug('---')
            logging.debug("Parameter: v_parm_dg_outcome_range \n<\n" + v_parm_dg_outcome_range + "\n>")
            logging.debug('---')
        v_list_outcome_range = json.loads(v_parm_dg_outcome_range)
        if int(v_parm_flag_loglevel) > 0:
            logging.debug('Parm   : ' + 'v_parm_dg_dmd_trends')
            logging.debug('List   : ' + str(v_list_dmd_trends))
            logging.debug('Type   : ' + str(type(v_parm_dg_outcome_range)))
            logging.debug('Count  : ' + str(len(v_parm_dg_outcome_range)))
            logging.debug('---')
...
if 1 == 1:
    ...
    if 1 == 1:
        if int(v_parm_flag_loglevel) > 0:
            logging.debug('---')
            logging.debug("Parameter: v_parm_dg_outcome_trigger \n<\n" + v_parm_dg_outcome_trigger + "\n>")
            logging.debug('---')
        v_list_outcome_trigger = json.loads(v_parm_dg_outcome_trigger)
        if int(v_parm_flag_loglevel) > 0:
            logging.debug('Parm   : ' + 'v_parm_dg_dmd_trends')
            logging.debug('List   : ' + str(v_list_dmd_trends))
            logging.debug('Type   : ' + str(type(v_parm_dg_outcome_trigger)))
            logging.debug('Count  : ' + str(len(v_parm_dg_outcome_trigger)))
            logging.debug('---')
...
if 1 == 1:
    ...
    if 1 == 1:
        if int(v_parm_flag_loglevel) > 0:
            logging.debug('---')
            logging.debug("Parameter: v_parm_dg_outcome_fix \n<\n" + v_parm_dg_outcome_fix + "\n>")
            logging.debug('---')
        v_list_outcome_fix = json.loads(v_parm_dg_outcome_fix)
        if int(v_parm_flag_loglevel) > 0:
            logging.debug('Parm   : ' + 'v_parm_dg_dmd_trends')
            logging.debug('List   : ' + str(v_list_dmd_trends))
            logging.debug('Type   : ' + str(type(v_parm_dg_outcome_fix)))
            logging.debug('Count  : ' + str(len(v_parm_dg_outcome_fix)))
            logging.debug('---')
...
if 1 == 1:
    ...
    # -
    v_delta_mode = os.environ.get('DATAGEN_COREDATA_DELTA') == '1'
    if v_delta_mode:
        v_path_items            = v_parm_path_items_base
        v_path_customers        = v_parm_path_customers_base
        v_path_matrix           = v_parm_path_matrix_merged_new
        v_path_coredata         = v_parm_path_coredata_delta
    else:
        v_path_items            = v_parm_path_items_base
        v_path_customers        = v_parm_path_customers_base
    ...
    if 1 == 2:
        logging.debug('-----')
        logging.debug("Start: " + v_temp_date_start )
        logging.debug("End  : " + v_temp_date_end )
        logging.debug(str(v_list_dates_history))
    ...
    if 1 == 2:
        ...
        logging.debug("Start: " + v_temp_date_start )
        logging.debug("End  : " + v_temp_date_end )
        logging.debug(str(v_list_dates_openorders))
    ...
    if 1 == 2:
        ...
        logging.debug("Start: " + v_temp_date_start )
        logging.debug("End  : " + v_temp_date_end )
        logging.debug(str(v_list_dates_future))

    # Delta mode: use period tracker start -> future_end only; skip history/openorders
    if v_delta_mode:
        with open(v_parm_path_period_tracker, 'r', encoding='utf-8') as f:
            v_delta_start_str = f.read().strip().split('\n')[0].strip()
        v_list_dates_history = pandas.DatetimeIndex([])
        v_list_dates_openorders = pandas.DatetimeIndex([])
        v_temp_date_start = datetime.strptime(v_delta_start_str, v_date_format).strftime('%m-%d-%Y')
        v_temp_date_end = v_date_future_end.strftime('%m-%d-%Y')
        v_list_dates_future = pandas.date_range(start=v_temp_date_start, end=v_temp_date_end, freq=v_parm_dg_period_type_incr_str)
    ...
    with open(v_path_matrix, mode='r') as v_fileHandle_matrix:
        ...
        # Test-only variable to limit output while testing a couple of combinations
        # Set to a very large number normally (larger than any reasonable matrix row-count).
        # If the flag is set to "1", then limit the counter to limit processing.
        v_rowCounter_matrix_limiter = 1000000
        if 1 == 2:
            v_rowCounter_matrix_limiter = 10
        ...
        for v_row_matrix in v_csv_file_matrix_reader:
            ...
            # ---
            if v_rowCounter_matrix > 1 and v_rowCounter_matrix <= v_rowCounter_matrix_limiter:

                # NORMAL OPERATION: load algorithm from matrix file
                if 1 == 1:
                    v_current_trend_type = v_row_matrix[4]

                # TEST OPERATION: random algorithm selection
                if 1 == 2:
                    # -
                    ...
                    v_trend_dict = v_list_dmd_trends[v_temp_rnd]
                    v_current_trend_type = v_trend_dict.get("TrendName", "") if isinstance(v_trend_dict, dict) else str(v_trend_dict)
                    # print (str(v_current_trend_type))

                # TEST LINE: here has a place to put a break to check variable values
                print(len(v_row_matrix))
                print(v_row_matrix)
                x = 1
                ...
                # -
                if (v_current_trend_type == "Intermittent01"):
                ...
                elif (v_current_trend_type == "Slope01"):
                ...
                elif (v_current_trend_type == "Seasonal01"):
                    ...
                    # -   sine oscillates between min and max: center = (min+max)/2, amplitude = (max-min)/2
                    ...
                    v_current_trend_seasonal_y = (v_current_trend_seasonal_min + v_current_trend_seasonal_max)/2 + ((v_current_trend_seasonal_max - v_current_trend_seasonal_min)/2) * math.sin( (v_current_trend_seasonal_x + v_current_trend_seasonal_phase) * math.pi )

                    # Set overall value for volume (regardless of method used)
                    v_current_calculated_volume_raw = v_current_trend_seasonal_y

                    # Set production capacity to a representative value about 3 years out (same x scale as sine: x increases by 2 per year)
                    v_capacity_estimate_x_value = 2 * 3  # 3 years => x = 6
                    v_current_inventory_capacity = (v_current_trend_seasonal_min + v_current_trend_seasonal_max)/2 + ((v_current_trend_seasonal_max - v_current_trend_seasonal_min)/2) * math.sin( (v_capacity_estimate_x_value + v_current_trend_seasonal_phase) * math.pi )

                elif (v_current_trend_type == "Combined01"):
                    ...
                    # Set production capacity to a representative value about 3 years out (line uses period count; seasonal uses x = 6 for 3 years)
                    v_capacity_estimate_x_value = v_current_trend_seasonal_periods_per_year * 3
                    v_current_inventory_capacity_line = (v_current_trend_line_m * v_capacity_estimate_x_value) + v_current_trend_line_b
                    v_capacity_estimate_x_seasonal = 2 * 3  # 3 years in sine x units
                    v_current_inventory_capacity_seasonal = (v_current_trend_seasonal_min + v_current_trend_seasonal_max)/2 + ((v_current_trend_seasonal_max - v_current_trend_seasonal_min)/2) * math.sin( (v_capacity_estimate_x_seasonal + v_current_trend_seasonal_phase) * math.pi )
                        ...
                        if 1 == 1:
                            for v_current_date_element in v_list_dates_history:
                                ...
                                if 1 == 2:
                                    v_tempstr = str(v_rowCounter_matrix)
                                    logging.debug('-----')
                                    logging.debug(v_tempstr.rjust(3,'0') + ", " + str(type(v_current_output_row)) + ", " + str(v_current_output_row) + " " + v_current_date_element.strftime("%Y-%m-%d"))
...
# ---
logging.debug("\n"*2)
logging.debug("Total new COREDATA rows created: " + str(v_rowCounter_coredata) )
logging.debug("\n"*2)
