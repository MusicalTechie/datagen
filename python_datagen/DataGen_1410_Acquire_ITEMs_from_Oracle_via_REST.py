# ==================================================
# = File:        DataGen_1410_Acquire_ITEMs_from_Oracle_via_REST.py
# = Purpose:     Execute a REST API call to acquire ITEM information from a designated Oracle Cloud Fusion environment
# ==================================================
# = Notes, Warnings, Requirements:
# = ------------------------------------------------
# =  - Uses config: v_parm_fusion_*, v_parm_path_items_from_source.
...
v_current_procedure_name = 'Acquire ITEMs from Oracle via REST'
...
v_api_rest_endpoint = "/fscmRestApi/resources/11.13.18.05/itemsLOV"
...
v_api_rest_response_items_array = []

v_response_limit = v_api_rest_rec_limit

# --- Acquire ITEMs in blocks until no more records ---
while True:
    ...
    v_total_items = v_json.get("totalResults", v_json.get("totalItems", -1))
    v_items = v_json.get("items", [])

    for v_item in v_items:
        v_api_rest_response_items_array.append(v_item)

    if not v_has_more or (v_total_items is not None and int(v_total_items) == 0):
        logging.debug("No more records (hasMore=" + str(v_has_more) + ", totalItems/totalResults=" + str(v_total_items) + ")")
        break
...
# --- Write all items and attributes to quote-delimited CSV (overwrite if exists) ---
v_output_path = v_parm_path_items_from_source
v_output_dir = os.path.dirname(v_output_path)
if v_output_dir:
    os.makedirs(v_output_dir, exist_ok=True)

if v_api_rest_response_items_array:
    v_keys_set = set()
    for v_item in v_api_rest_response_items_array:
        if isinstance(v_item, dict):
            v_keys_set.update(v_item.keys())
    v_all_keys = sorted(v_keys_set) if v_keys_set else (sorted(v_api_rest_response_items_array[0].keys()) if v_api_rest_response_items_array else [])
    ...
    with open(v_output_path, mode="w", newline="", encoding="utf-8") as v_f:
        ...
        for v_item in v_api_rest_response_items_array:
            if isinstance(v_item, dict):
                v_row = [v_item.get(k, "") for k in v_all_keys]
                v_writer.writerow(v_row)
            else:
                v_writer.writerow([str(v_item)])

    logging.debug("Wrote " + str(len(v_api_rest_response_items_array)) + " items to " + str(v_output_path))
else:
    with open(v_output_path, mode="w", newline="", encoding="utf-8") as v_f:
        v_writer = csv.writer(v_f, quoting=csv.QUOTE_ALL, delimiter=",")
        v_writer.writerow([])
    logging.debug("No items to write; created empty file: " + str(v_output_path))

logging.debug("Total ITEM rows acquired via REST: " + str(len(v_api_rest_response_items_array)))
logging.debug("\n" * 2)
logging.debug("==================================================")
