# ==================================================
# = File:        DataGen_1390_Compose_FBDI_Booking_History.py
# = Purpose:     Select data from COREDATA (merged_new) to compose an FBDI-formatted CSV
# =              for loading into Oracle Fusion SCM; save to v_parm_path_fbdi_booking_new.
# ==================================================
# = Notes, Warnings, Requirements:
# = ------------------------------------------------
# =  - Input: v_parm_path_coredata_merged_new; Output: v_parm_path_fbdi_booking_new.
# =
# = Dates:
# = ------------------------------------------------
# =  - Created:     2026-MAR-07
...
v_current_procedure_name = 'Compose FBDI: Booking History (merged_new)'
...
v_path_output = v_parm_path_fbdi_booking_new
...
with open(v_path_coredata, mode='r', newline='', encoding='utf-8') as v_fileHandle_input:
    ...
    v_fbdi_output_dir = os.path.dirname(v_path_output)
    if v_fbdi_output_dir:
        os.makedirs(v_fbdi_output_dir, exist_ok=True)

    with open(v_path_output, mode='w', newline='', encoding='utf-8', buffering=1024 * 1024) as v_filename_output:
