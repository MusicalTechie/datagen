...
# ---
v_current_procedure_name = 'Generate ITEMS: INITIAL'
...
if 1 == 1:
    ...
    # v_parm_flag_loglevel = 1
    if 1 == 1:
        v_list_cust_hier_names = json.loads(v_parm_dg_cust_hier_names)
        if int(v_parm_flag_loglevel) > 0:
            logging.debug('---')
            logging.debug('Parm   : ' + 'v_parm_dg_cust_hier_names')
            logging.debug('List   : ' + str(v_list_cust_hier_names))
            logging.debug('Type   : ' + str(type(v_list_cust_hier_names)))
            logging.debug('Count  : ' + str(len(v_list_cust_hier_names)))
            logging.debug('---')
        for v_parent_element in v_list_cust_hier_names:
            if int(v_parm_flag_loglevel) > 0:
                logging.debug(" - Parent row: " + str(v_parent_element))
            for v_child_element in v_parent_element:
                if int(v_parm_flag_loglevel) > 0:
...
# ---
# - Generate initial list of ITEMS (using shared hierarchy CSV generator)
# ---
from DataGen_shared_hierarchy_csv import generate_hierarchy_csv

if 1 == 1:
    v_path_items = v_parm_path_items_base
    logging.debug("Writing ITEMS CSV to: " + str(v_path_items))
...
# ---
logging.debug("\n"*2)
logging.debug("Total new ITEM rows created: " + str(v_rowCounter_items) )
logging.debug("\n"*2)
