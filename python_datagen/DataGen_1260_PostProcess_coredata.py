...
# ==================================================
# = File:	    DataGen_1260_PostProcess_coredata.py
...
if 1 == 1:
    ...
    if 1 == 1:
        ...
        if int(v_parm_flag_loglevel) > 0:
            logging.debug('Parm   : ' + 'v_parm_dg_outcome_range')
            logging.debug('List   : ' + str(v_list_outcome_range))
            logging.debug('Type   : ' + str(type(v_list_outcome_range)))
            logging.debug('Count  : ' + str(len(v_list_outcome_range)))
            logging.debug('---')
...
if 1 == 1:
    ...
    if 1 == 1:
        ...
        if int(v_parm_flag_loglevel) > 0:
            logging.debug('Parm   : ' + 'v_parm_dg_outcome_trigger')
            logging.debug('List   : ' + str(v_list_outcome_trigger))
            logging.debug('Type   : ' + str(type(v_list_outcome_trigger)))
            logging.debug('Count  : ' + str(len(v_list_outcome_trigger)))
            logging.debug('---')
...
if 1 == 1:
    ...
    if 1 == 1:
        ...
        if int(v_parm_flag_loglevel) > 0:
            logging.debug('Parm   : ' + 'v_parm_dg_outcome_fix')
            logging.debug('List   : ' + str(v_list_outcome_fix))
            logging.debug('Type   : ' + str(type(v_list_outcome_fix)))
            logging.debug('Count  : ' + str(len(v_list_outcome_fix)))
            logging.debug('---')
...
if 1 == 1:
    ...
    # -
    if os.environ.get('DATAGEN_POSTPROCESS_MERGED_NEW') == '1':
        v_path_coredata          = v_parm_path_coredata_merged_new
        v_path_coredata2         = v_parm_path_coredata2_merged_new
    else:
    ...
    # ---
    # - Prepare a FIFO queue (list) for rows of data.
    ...
    with open(v_path_coredata, mode='r', newline='') as v_fileHandle_coredata:
        ...
        if int(v_parm_flag_loglevel) > 0:
            logging.debug(" ")
            logging.debug("Opening READ file,  COREDATA ...")
            sleep(1)
        ...
        with open(v_path_coredata2, mode='w', newline='') as v_fileHandle_coredata2:
            ...
            if int(v_parm_flag_loglevel) > 0:
                logging.debug(" ")
                logging.debug("Opening WRITE file, COREDATA2...")
                sleep(1)
            ...
            for v_row_coredata in v_csv_file_coredata_reader:
                ...
                if 1 == 2:
                    logging.debug('-----')
                    logging.debug(v_tempstr.rjust(3,'0') + ", " + str(type(v_row_coredata2)) + ", " + str(v_row_coredata2) + " ")
                    sleep(1)
                ...
                if v_rowCounter_coredata2 > 1:
                    ...
                    if 1 == 1:
                        if int(v_parm_flag_loglevel) > 0:
                            logging.debug('Working on row: [' + str(v_rowCounter_coredata2) + ']')
                            logging.debug(' - <' + str(v_row_coredata) + '>')
                            logging.debug(' ')
                            sleep(1)
                    ...
                    v_current_output_row = '{ ' + '"' + v_f01_name + '"' + ": " + '"' + v_f01_value + '"' \
                                           ...
                                           + ', "' + v_f15_name + '"' + ": " + '"' + "safetystock02_replace_me" + '"' \
                                           + ', "' + v_f16_name + '"' + ": " + '"' + "safetystock03_replace_me" + '"' \
                    ...
                    # ---
                    # - Append current row to FIFO; when we have 6, pop oldest, replace placeholders, write.
                    # ---
                    v_fifo_rows.append(v_current_output_row)

                    if len(v_fifo_rows) >= 6:
                        row_to_write = v_fifo_rows.pop(0)

                        if int(v_parm_flag_loglevel) > 0:
                            logging.debug('- unadjusted row : [' + str(row_to_write) + ']')
                            logging.debug(' ')
                            sleep(1)

                        v_safety_stock_rolling_avg_altered = str(int(float(v_rolling_safety_stock) * v_outcome_fix))
                        v_tmp_string = row_to_write.replace("safetystock02_replace_me", str(v_safety_stock_rolling_avg_altered) )
                        v_tmp_string = v_tmp_string.replace("safetystock03_replace_me", str(0) )
                        v_tmp_string = v_tmp_string.replace("outcome03_replace_me", "" )

                        if int(v_parm_flag_loglevel) > 0:
                            logging.debug('- adjusted row : [' + str(v_tmp_string) + ']')
                            logging.debug(' ')
                            sleep(1)

                        v_current_output_row_json = json.loads(v_tmp_string)
            ...
            # ---
            # - Write out the remaining rows left in the FIFO queue (no rolling avg; use 0 and empty string)
            # ---
            for row_str in v_fifo_rows:
                if row_str != "":
                    v_tmp_string = row_str.replace("safetystock02_replace_me", str(0))
                    v_tmp_string = v_tmp_string.replace("safetystock03_replace_me", str(0))
                    v_tmp_string = v_tmp_string.replace("outcome03_replace_me", "" )
                    v_current_output_row_json = json.loads(v_tmp_string)
                    v_csv_file_coredata2_writer.writerow(v_current_output_row_json.values())
...
# ---
logging.debug("\n"*2)
logging.debug("Total new coredata2 rows created    : " + str(v_rowCounter_coredata2) )
logging.debug("Total unique ITEM-LOC keys processed: " + str(v_keyCounter) )
logging.debug("\n"*2)
