...
# ---
v_current_procedure_name = 'Full Dataset: Post-Process COREDATA: Rolling calculations'
    ...
    with open(v_path_coredata, mode='r', newline='') as v_fileHandle_coredata:
        ...
        with open(v_path_coredata2, mode='w', newline='') as v_fileHandle_coredata2:
            ...
            for v_row_coredata in v_csv_file_coredata_reader:
                ...
                if v_rowCounter_coredata2 > 1:
                    ...
                    if v_rowCounter_coredata2 >= 7:
                        ...
                        # TEST: This variant allows visibility into the rolling average, a "fix", and the net result.
                        # v_tmp_string = v_coredata2_row_06.replace("safetystock02_replace_me", str(v_rolling_safety_stock) )
                        # v_tmp_string = v_tmp_string.replace("safetystock03_replace_me", str(v_rolling_safety_stock_mod) )
                        # v_tmp_string = v_tmp_string.replace("outcome03_replace_me", str(v_safety_stock_rolling_avg_altered) )

                        # FINAL: This variant allows visibility into the rolling average, a "fix", and the net result.
                        v_tmp_string = v_coredata2_row_06.replace("safetystock02_replace_me", str(v_safety_stock_rolling_avg_altered) )
                        v_tmp_string = v_tmp_string.replace("safetystock03_replace_me", str(0) )
                        v_tmp_string = v_tmp_string.replace("outcome03_replace_me", "" )

                        # Update the variable that will be written
            ...
            if v_coredata2_row_05 != "":
                v_tmp_string = v_coredata2_row_05.replace("safetystock02_replace_me", str(0))
                v_tmp_string = v_tmp_string.replace("safetystock03_replace_me", str(0))
                v_tmp_string = v_tmp_string.replace( "outcome03_replace_me", "" )
