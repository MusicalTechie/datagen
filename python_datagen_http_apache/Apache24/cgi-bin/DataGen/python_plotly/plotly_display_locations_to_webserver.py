...
#---
#- plotly_display_locations_to_webserver.py
#---
...
# df = pd.read_csv('http://172.26.11.182/LOCATIOS_base.csv')

# ENABLED
# Get the data from a local path directly
df = pd.read_csv('C:/Apache/Apache24/cgi-bin/DataGen/data_results/base/CUSTOMERS_base.csv')
...
v_graphic_object = pgo.Figure(data=[pgo.Table(
    header=dict(values=list(df.columns),
                ...
                align='left'),
    cells=dict(values=[df.WorldRegion,df.Country,df.Region,df.SubRegion,df.ShipToLocation],
               fill_color='lavender',
               align='left'))
])
