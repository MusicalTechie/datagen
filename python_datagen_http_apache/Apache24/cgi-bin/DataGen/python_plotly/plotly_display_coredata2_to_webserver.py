...
#---
#- plotly_display_coredata2_to_webserver.py
#---
...
# df = pd.read_csv('http://172.26.11.182/COREDATA2_base.csv')

# ENABLED
# Get the data from a local path directly
df = pd.read_csv('C:/Apache/Apache24/cgi-bin/DataGen/data_results/base/COREDATA2_base.csv')
...
v_graphic_object = pgo.Figure(data=[pgo.Table(
    header=dict(values=list(df.columns),
                ...
                align='left'),
    cells=dict(values=[df.Period,df.Item,df.Location,df.Orders_Complete,df.Orders_Open,df.Orders_Cancelled           \
                        ,df.Orders_Returned,df.Forecast01,df.Forecast02,df.Inventory_starting,df.Inventory_capacity  \
                        ,df.Inventory_production,df.Inventory_end,df.SafetyStock01,df.SafetyStock02,df.SafetyStock03 \
                        ,df.Net_Requirement,df.Outcome01,df.Outcome02,df.Outcome03,df.Trend_type,df.Trend_factor1    \
                        ,df.Trend_factor2,df.Trend_factor3,df.Trend_factor4,df.Trend_factor5,df.Last_Period          \
                        ,df.Last_X_Value,df.Last_Y_Value],
               fill_color='lavender',
               align='left'))
])
