...
If a typical DataGen run is under ~15 minutes, Lambda (or Step Functions + Lambda) is a good fit; if it’s longer or needs more disk, use Fargate or EC2 with a small runner service and EFS (or S3) for config and outputs. **DataGen_1410_Acquire_ITEMs_from_Oracle_via_REST** and **DataGen_1420_Acquire_CUSTOMERs_from_Oracle_via_REST** (REST clients to Oracle Fusion to retrieve items and shipping customers) are typically short-running and can be exposed as API actions (e.g. “Acquire items from Fusion", "Acquire customers from Fusion”).
